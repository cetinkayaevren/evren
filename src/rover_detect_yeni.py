...
def main():
	...
	greenLower = (28,48, 64)  # Less accurate -> (29,86,6)  kullanilan(29,50,150)94
	greenUpper = (78,255,255)  #kullanilan   (64,255,255)
	...
	while not rospy.is_shutdown():#Use this command for detection over a video instead 'True' --> 'camera.isOpened()'
		...
		frame_gau_blur = cv2.GaussianBlur(frame, (5, 5), 2) #cv2.BORDER_DEFAULT
		...
		mask = cv2.dilate(mask, None, iterations=3) #3
		...
		canny_edge = cv2.morphologyEx(canny_edge, cv2.MORPH_OPEN, kernel)
		
		# applying HoughCircles
		rows=blue_s_gray.shape[0]
		circles = cv2.HoughCircles(canny_edge, cv2.HOUGH_GRADIENT, dp=2, minDist=9, param1=10, param2=6, minRadius=0, maxRadius=0) #param2 yuksekse detection kaliteli uzaklik kalitesiz, vs
		...
		if len(cnts) > 0 and circles is not None:
			difference_x=mean_x-old_x
			difference_y=mean_y-old_y
			if (-50 <difference_x - old_difference_x< 50 and -50<difference_y - old_difference_y<50):
				count += 1
				count2 = 0
			...
			if radius > 0.2 and  ball_detected is True:
				...
				mean_x=coordinates_x[0]   #degistirildi
				mean_y=coordinates_y[0]
		...
		#cv2.imshow("blurrred",frame_gau_blur)
